...
abstract class PostgresTable<T extends PostgresDataRow> {
  String get tableName;
  T createRow(Map<String, dynamic> data);

  PostgrestFilterBuilder<T> _select<T>() =>
      FFPostgresClient.instance.from(tableName).select<T>();
  ...
  Future<T> insert(Map<String, dynamic> data) => FFPostgresClient.client
      .from(tableName)
      .insert(data)
      .select<PostgrestMap>()
      .limit(1)
  ...
  Future<List<T>> update({
    ...
    final update =
        matchingRows(FFPostgresClient.instance.from(tableName).update(data));
    if (!returnRows) {
      await update;
      return [];
    }
  ...
  Future<List<T>> delete({
    ...
    final delete =
        matchingRows(FFPostgresClient.instance.from(tableName).delete());
    if (!returnRows) {
      await delete;
      return [];
    }
